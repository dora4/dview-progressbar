...
import dora.widget.progressbar.R

class DoraProgressBar @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null,
    defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {

    private val bounds = RectF()
    private var sweepAnimator: ObjectAnimator? = null
    private var angleAnimator: ObjectAnimator? = null
    ...
    private val angleProperty: Property<DoraProgressBar, Float> =
        object : Property<DoraProgressBar, Float>(
            Float::class.java, "angle"
        ) {
            override fun get(`object`: DoraProgressBar): Float {
                return `object`.currentGlobalAngle
            }

            override fun set(`object`: DoraProgressBar, value: Float) {
                `object`.currentGlobalAngle = value
            }
        }

    private val sweepProperty: Property<DoraProgressBar, Float> =
        object : Property<DoraProgressBar, Float>(
    ...
    init {
        ...
        val a = context.obtainStyledAttributes(attrs, R.styleable.DoraProgressBar, defStyleAttr, 0)
        progressWidth = a.getDimension(
            R.styleable.DoraProgressBar_dview_pb_progressWidth,
            10 * density
        )
        progressColor = a.getColor(R.styleable.DoraProgressBar_dview_pb_progressColor, Color.BLACK);
        a.recycle()
        colors = IntArray(4)
